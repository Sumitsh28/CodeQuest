  ...
  async function handleRegister(ev) {
    ...
    if (response.ok) {
      await signIn("credentials", {
        email,
        password,
        callbackUrl: isLogin ? "/" : "/edit-profile",
      });
    } else {
      setError(true);
    }
  ...
  return (
    <section className="flex flex-col justify-center items-center min-h-[72vh]">
      ...
      <Link href="/" className="flex justify-center items-center mb-12 gap-5">
        <img src="/nf.png" className="w-[200px] h-[200px] glow-effect" />
      </Link>
      <form
      ...
      >
        ...
        <div className="text-center mb-4">
          <button
            type="submit"
            disabled={loginInProgress}
            className="inline-flex items-center px-8 py-2 mt-4 bg-[#4BD4A3] text-white font-medium rounded-lg mx-auto disabled:cursor-not-allowed"
          >
            {loginInProgress ? (
              <img
                src="loader.svg"
