      ...
      <div className="w-full max-w-6xl p-4 my-6 flex gap-12 mx-auto flex-col">
        <div className="bg-dark-1 text-white h-fit mx-auto w-full max-w-sm rounded-xl p-2 flex flex-col">
          <div className="flex justify-between">
            <button
              className={`option-button w-full py-3 px-2 rounded-t-xl ${
                selectedOption === "top"
                  ? "bg-white text-dark-1 selected font-medium"
                  : ""
              }`}
          ...
          <div className="bg-white py-8 px-2 rounded-b-xl">
            <h1 className="text-2xl font-bold text-center text-dark-1">
              {pageTitle}
            </h1>
          </div>
        </div>
        <ol className="w-full max-w-6xl flex flex-wrap items-start mx-auto justify-center gap-4">
          {loading && (
            <img
              src="/loader.svg"
              className="w-12 h-12 my-12 object-contain mx-auto"
            />
          )}
          {!loading &&
            (pageTitle !== "Job Listings" ? (
              stories.map((story, index) => (
                <a
                  key={story.id}
                  className="flex items-center w-full max-w-md p-4 rounded-xl hover:mt-[-7px] bg-light-2 hover:bg-dark-1 group transition-all ease-in"
                  href={story.url}
                  target="_blank"
                ...
                >
                  ...
                  <div className="">
                    <h2 className="group-hover:text-white transition-all ease-in">
                      {story.title}
                    </h2>
                    <div className="flex flex-wrap gap-1 items-end">
                      <span className="text-orange-600 group-hover:text-orange-400 transition-all ease-in">
                        {story.by ? `source: ${story.by}` : ""}
                      </span>
                      <span className="text-sm text-gray-700 group-hover:text-gray-400 transition-all ease-in">
                        {" "}
                        • {story.score} points
                      </span>
                      <span className="text-sm text-gray-700 group-hover:text-gray-400 transition-all ease-in">
              ...
              <div className="flex items-center w-full max-w-md p-4 rounded-xl hover:mt-[-7px] bg-light-2 hover:bg-dark-1 group transition-all ease-in justify-center">
                <h2 className="group-hover:text-white transition-all ease-in">
                  First Let Me Get One!!!🥲
                </h2>
              </div>
            ))}
        </ol>
