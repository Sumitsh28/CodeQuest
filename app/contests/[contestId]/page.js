...
const page = () => {
  ...
  const rankings = [
    { username: "Sumit Shandillya", score: 18, finish_time: "0:15:16" },
    { username: "Sumit Shandillya", score: 18, finish_time: "0:15:16" },
    { username: "Sumit Shandillya", score: 18, finish_time: "0:15:16" },
    { username: "Sumit Shandillya", score: 18, finish_time: "0:15:16" },
    { username: "Sumit Shandillya", score: 18, finish_time: "0:15:16" },
    ...
    <div className="w-full flex flex-col gap-5 max-w-6xl mx-auto px-2 mt-3 mb-10">
      ...
      <div className="flex gap-5 max-md:flex-col max-md:items-center ">
        <div className="flex-grow">
          ...
          >
            ...
            />
            <div className="w-full py-5 flex justify-between mx-5 text-white">
              {isUpcoming ? (
          ...
          <div className="bg-[#9AA3AF] rounded-r-full pl-4 pr-10 py-4 mb-5 w-fit">
            <h1 className="text-2xl">
              Weekly Contest {params.contestId.slice(15)}
            </h1>
          </div>

          <h2 className="my-2 text-white">Rules:</h2>
          <ul className="text-white">
            <li>
              There will be four problems in the contest and each will have its
            ...
            <li>
              To ensure the fairness of the contest, Code Quest will hide some
              test cases during the contest. When users submit incorrect
              submissions, Code Quest will not show the hidden test cases to the
              users.
            </li>
            <li>
        ...
        <div className="w-full max-w-[300px] bg-[#D9D9D9] rounded-lg h-fit shadow-lg pb-2">
      ...
      {!isUpcoming && (
        <div className="mt-10 flex items-start gap-5 max-md:flex-col">
          <div className="w-full rounded-lg hover:shadow-lg transition-all bg-[#D9D9D9]">
            <div className="bg-[#9AA3AF] flex justify-between items-center px-4 py-2.5 rounded-t-lg">
            ...
            {problems.map((prob, index) => (
              <Link
                key={index}
                href={`/contests/${params.contestId}/${prob.title
                  .toLowerCase()
                  .split(" ")
                  .join("-")}`}
              ...
              >
                ...
                <p className="text-sm bg-gray-400 text-light-1 py-0.5 px-2 rounded-lg">
                  {prob.score}
                </p>
              </Link>
            ))}
          ...
          <div className="w-full rounded-lg hover:shadow-lg transition-all">
            <div className="bg-[#9AA3AF] px-4 py-2.5 rounded-t-lg">
              <p className="font-medium">Ranking</p>
            </div>

            <div className="flex bg-light-4 px-4 py-2.5">
              <p className="w-[15%] font-medium max-xs:text-sm text-center">
                Rank
              </p>
              <p className="w-[40%] font-medium max-xs:text-sm text-center">
                Name
              </p>
              <p className="w-[15%] font-medium max-xs:text-sm text-center">
                Score
              </p>
              <p className="w-[30%] font-medium max-xs:text-sm text-center">
                Finish Time
              </p>
            </div>
            ...
            {rankings.map((ranking, index) => (
              <div
                key={index}
                className="flex bg-[#faf9f9] px-4 py-2.5 hover:bg-[#fff]"
              >
                <p className="text-sm w-[15%] text-center">{index + 1}</p>
                <Link
                  href={`/profile/${ranking.username}`}
                  className="text-sm w-[40%] hover:text-blue-500 text-center"
                >
                  {ranking.username}
                </Link>
                <p className="text-sm w-[15%] text-center">{ranking.score}</p>
                <p className="text-sm w-[30%] text-center">
                  {ranking.finish_time}
                </p>
              </div>
            ))}

            <div className="hover:bg-light-4 bg-[#9AA3AF] text-center rounded-b-lg flex">
              <Link
